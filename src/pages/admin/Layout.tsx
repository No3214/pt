...
const tabs = [
  ...
  ), label: 'Değerlendirme' },  { to: 'builder', icon: (
    <svg className="w-[18px] h-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
export default function AdminLayout() {
  ...
  return (
    <div className={`min-h-screen font-body ${dm ? 'bg-[#050505]' : 'bg-[#F7F5F2]'}`}>
      {/* ═══ Top Bar ═══ */}
      <header className={`fixed top-0 left-0 right-0 z-[100] backdrop-blur-2xl border-b transition-all duration-500 ${dm ? 'bg-[#050505]/80 border-white/[0.04]' : 'bg-white/70 border-black/[0.03]'}`}>        <div className="max-w-[1600px] mx-auto px-5 md:px-8 h-16 flex justify-between items-center">
              ...
              <nav className="flex flex-col gap-0.5 flex-1">
                {tabs.map(t => (
                  <NavLink key={t.to} to={t.to} onClick={() => setSidebarOpen(false)}
                    ...
                    <span className="flex-shrink-0 opacity-80">{t.icon}</span>
                    <span>{t.label}</span>
                  </NavLink>
                ))}
              </nav>
