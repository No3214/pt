      ...
      <header className={`fixed top-0 left-0 right-0 z-[100] backdrop-blur-xl border-b py-4 ${dm ? 'bg-[#0a0a0a]/90 border-white/5' : 'bg-white/80 border-black/[0.04]'}`}>
        <div className="max-w-[1500px] mx-auto px-6 flex justify-between items-center">
          <div className="flex items-center gap-4">
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="md:hidden bg-transparent border-none cursor-pointer p-2" aria-label="Menü">
              <div className="space-y-[4px]">
                <motion.span animate={sidebarOpen ? { rotate: 45, y: 5.5 } : { rotate: 0, y: 0 }} className={`block w-5 h-[1.5px] rounded-full ${dm ? 'bg-white' : 'bg-[#1C1917]'}`} />
                <motion.span animate={sidebarOpen ? { opacity: 0 } : { opacity: 1 }} className={`block w-5 h-[1.5px] rounded-full ${dm ? 'bg-white' : 'bg-[#1C1917]'}`} />
                <motion.span animate={sidebarOpen ? { rotate: -45, y: -5.5 } : { rotate: 0, y: 0 }} className={`block w-5 h-[1.5px] rounded-full ${dm ? 'bg-white' : 'bg-[#1C1917]'}`} />
              </div>
            </button>
          ...
          <div className="flex gap-3 items-center">
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={toggleDarkMode}
              className={`w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 border-none cursor-pointer ${dm ? 'bg-white/5 text-white' : 'bg-black/5 text-[#1C1917]'}`} aria-label="Tema">
            ...
            <button onClick={logoutAdmin} className={`rounded-full px-5 py-2 text-[0.78rem] border cursor-pointer bg-transparent transition-all duration-300 ${dm ? 'border-white/10 text-white/60 hover:text-white hover:border-white/30' : 'border-black/10 text-[#1C1917]/50 hover:text-[#1C1917] hover:border-black/25'}`}>
              Çıkış
            </button>
          </div>
        </div>
          ...
          {(sidebarOpen || true) && (
            <motion.aside initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }}
              ...
              <div className={`relative z-10 rounded-2xl p-5 ${dm ? 'bg-[#111] border border-white/5' : 'bg-white border border-black/[0.04]'}`}>
                ...
                <nav className="flex flex-col gap-0.5">
                  {tabs.map(t => (
                    <NavLink key={t.to} to={t.to} onClick={() => setSidebarOpen(false)}
                      className={({ isActive }) =>
                        `flex items-center gap-3 px-4 py-3 rounded-xl no-underline text-[0.82rem] transition-all duration-300 ${isActive ? `font-medium ${dm ? 'bg-terracotta/10 text-terracotta' : 'bg-terracotta/8 text-terracotta'}` : `${dm ? 'text-white/40 hover:text-white/70 hover:bg-white/[0.03]' : 'text-[#1C1917]/40 hover:text-[#1C1917]/70 hover:bg-black/[0.02]'}`}`
                      }>
                ...
                <div className={`mt-6 pt-4 border-t text-center ${dm ? 'border-white/5' : 'border-black/[0.04]'}`}>
                  <p className={`text-[0.6rem] uppercase tracking-widest ${dm ? 'text-white/15' : 'text-[#1C1917]/15'}`}>v2.0 — God Tier</p>
                </div>
              </div>
            </motion.aside>
          )}
      ...
      <AnimatePresence>
        {toastMsg && (
          <motion.div
            ...
            className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-[200] px-6 py-3.5 rounded-full shadow-2xl text-sm font-medium ${dm ? 'bg-white text-[#1C1917] shadow-black/40' : 'bg-[#1C1917] text-white shadow-black/20'}`}>
